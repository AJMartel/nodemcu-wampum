return function (connection, req, args)
    ...
    if req.method == "GET" then
        dofile("http_httpgui-form.lc")(req.method, connection, httpservConfig)
        collectgarbage()
    elseif req.method == "POST" then      
        local rd = req.getRequestData()
        local badvalues = dofile("http_httpgui-validate.lc")(rd)
        collectgarbage()
        ...
        if next(badvalues) == nil then
            if next(rd) ~= nil then
                ...
                -- fix strings
                dofile("http_httpgui-requote.lc")(rd)
                collectgarbage()
            ...
            dofile("http_httpgui-form.lc")(req.method, connection, httpservConfig, rd, badvalues)
        end
    else
